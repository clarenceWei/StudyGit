<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="HiringInfoManage.aspx.cs" Inherits="Nui.MallInfo.Web.ManageHTML.HiringInfoManage" %>
                        ...
                        <asp:TemplateField HeaderText="职位">
                            <ItemTemplate>
                                <asp:HyperLink ID="HyperLink1" runat="server" 
                                    NavigateUrl='<%# Eval("ID", "ADDHiringInfo.aspx?id={0}") %>' ToolTip='<%#Eval("Position") %>' Text='<%# Eval("Position").ToString().Length>35?Eval("Position").ToString().Substring(0,32)+"...":Eval("Position") %>'></asp:HyperLink>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:BoundField DataField="Department" HeaderText="部门" />
                        <asp:BoundField DataField="Hiring_Number" HeaderText="招聘人数" />
                        <asp:BoundField DataField="AddDate" HeaderText="添加时间" 
                            DataFormatString="{0:yyyy-MM-dd}" />
                        <asp:TemplateField HeaderText="是否有效">
                            <ItemTemplate>
                                <asp:Label ID="Label1" runat="server" Text='<%#Eval("IsValid").ToString()=="0"?"是":"否" %>'></asp:Label>
                            </ItemTemplate>
<%--                            <EditItemTemplate>
                                <asp:TextBox ID="TextBox1" runat="server" Text='<%# Bind("IsValid") %>'></asp:TextBox>
                            </EditItemTemplate>--%>
                        </asp:TemplateField>
